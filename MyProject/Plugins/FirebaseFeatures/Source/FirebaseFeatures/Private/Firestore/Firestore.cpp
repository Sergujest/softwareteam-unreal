	...
	const FFirestoreSetOptions& Options)
{
	...
	if (Transaction && Document)
	{
		...
		Transaction->Set(*Document, MoveTemp(Values), Options);
	}
#endif // WITH_FIREBASE_FIRESTORE
}
	...
	const TMap<FString, FFirestoreFieldValue>& Data)
{
	...
	if (Transaction && Document)
	{
		...
		Transaction->Update(*Document, MoveTemp(Values));
	}
#endif // WITH_FIREBASE_FIRESTORE
}
	...
	EFirestoreError& ErrorCode, FString& ErrorMessage)
{
	...
	if (Transaction && Document)
	{
		...
		std::string ErrMsg;
		Snap = Transaction->Get(*Document, &Err, &ErrMsg);
	...
	const FFirestoreSetOptions& Options)
{
	...
	if (Document)
	{
		...
		Batch->Set(*Document, MoveTemp(RawData), Options);
	}
#endif // WITH_FIREBASE_FIRESTORE
	...
	const TMap<FString, FFirestoreFieldValue>& Data)
{
	...
	if (Document)
	{
		...
		Batch->Update(*Document, MoveTemp(RawData));
	}
#endif // WITH_FIREBASE_FIRESTORE
...
firebase::firestore::Firestore* UFirestore::GetFirestore()
{
	...
	return firebase::firestore::Firestore::GetInstance(firebase::App::GetInstance());
}
#endif // WITH_FIREBASE_FIRESTORE
...
#if WITH_FIREBASE_FIRESTORE
	*Collection->GetQuery() = GetFirestore()->Collection(TCHAR_TO_UTF8(*CollectionPath));
#endif // WITH_FIREBASE_FIRESTORE
...
#if WITH_FIREBASE_FIRESTORE
	Document->Reference = GetFirestore()->Document(TCHAR_TO_UTF8(*DocumentPath));
#endif // WITH_FIREBASE_FIRESTORE
