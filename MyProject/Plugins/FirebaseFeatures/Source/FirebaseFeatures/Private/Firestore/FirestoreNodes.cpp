...
UCollectionReferenceAddProxy* UCollectionReferenceAddProxy::Add(UFirestoreCollectionReference* const DocumentReference, const TMap<FString, FFirestoreFieldValue>& Data)
{
	ThisClass* const Proxy = NewObject<ThisClass>();
...
{
	BROADCAST_RESULT(OnDeleted, Document);
}
...
void UQueryGetProxy::Activate()
{
	if (!Query)
	{
		...
		OnTaskOver(EFirestoreError::InvalidArgument, TArray<FFirestoreDocumentSnapshot>());
		return;
	}
...
void UQueryGetProxy::OnTaskOver(const EFirestoreError Error, const TArray<FFirestoreDocumentSnapshot>& DocumentSnapshots)
{
	(Error == EFirestoreError::Ok ? OnGot : OnError).Broadcast(Error, DocumentSnapshots);
	SetReadyToDestroy();
}
