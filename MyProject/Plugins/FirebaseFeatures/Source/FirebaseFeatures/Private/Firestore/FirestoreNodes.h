...
public:
	/**
	 * Will be called with the DocumentReference of the
	 * newly created document.
	*/
	UPROPERTY(BlueprintAssignable)
	FDynMultDocumentCallback OnDeleted;
	UPROPERTY(BlueprintAssignable)
...
public:
	...
	static UCollectionReferenceAddProxy* Add(UFirestoreCollectionReference* const DocumentReference, const TMap<FString, FFirestoreFieldValue>& Data);
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FDynMultDocumentsCallback, const EFirestoreError, Error, const TArray<FFirestoreDocumentSnapshot>&, DocumentSnapshots);
...
private:
	void OnTaskOver(const EFirestoreError Error, const TArray<FFirestoreDocumentSnapshot>& DocumentSnapshots);
